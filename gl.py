...
from mathLib import mm, subtractVectors, crossProduct, normV, inverse
...
class Renderer(object):
    ...
    def glLookAt(self, eye, camPosition = V3(0,0,0)):
        forward = subtractVectors(camPosition, eye)
        forward = normV(forward)
    ...
    def glCamTransform(self, vertex):
        ...
        vt = mm(self.viewportMatrix, mm(self.projectionMatrix, mm(self.viewMatrix, v)))
        vf = V3(vt[0][0] / vt[3][0], 
                vt[1][0] /vt[3][0], 
                vt[2][0] /vt[3][0])
    ...
    def glTriangle_bc(self, A, B, C, verts = (), texCoords = (), normals = (), clr = None):
        ...
        triangleNormal = crossProduct( subtractVectors(verts[1], verts[0]), subtractVectors(verts[2],verts[0]))
        # normalizar
